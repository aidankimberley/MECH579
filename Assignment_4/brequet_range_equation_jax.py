...
def C_Dw(v,h):
    ...
    return 10*(jnp.atan(10*((v/(343*0.7))**2-1))+jnp.pi/2)
...
def C_D(v,h,weight,S):
    ...
    AR = 10
    coeff_drag = 0.5/60
    coeff_drag += C_L(v,h,weight,S)**2/(jnp.pi*e*AR)
...
def range(v,h,fuel_percentage,S):
    ...
    W_i = W_f + W_fuel
    total_range = v/ct(v,h,weight,S)*C_L(v,h,weight,S)/C_D(v,h,weight,S)*jnp.log(W_i/W_f)
...
v_axis = jnp.linspace(0.1,300,1000) # divide by zero error at 0.0, setting to 0.1
h_axis = jnp.linspace(0.1,25000,1000)

V,H = jnp.meshgrid(v_axis,h_axis)
...
i,j = jnp.where(j    max(range_mesh)==range_mesh)
