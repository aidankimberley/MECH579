...
def brequet_range(V,ct,CL,CD,Wi,Wf):
    return V/ct*CL/CD*(jnp.log(Wi/Wf))
...
get_rho = lambda h: 1.2*jnp.maximum(1-0.0065*h/288, 0.01)**5.26  # Clamp to avoid negative base
get_CD = lambda CD0,CL,AR,e,CWD: CD0 + CL**2/jnp.pi/AR/e + CWD
get_CWD = lambda V,c: 10*(jnp.arctan(10*((V/0.7/c)**2-1))+jnp.pi/2)
get_ct = lambda m_dot_f, T : m_dot_f/T + 1e-5
...
#%%
#Plot design space of the range of the aircraft (moved to __main__ section)
...
def brequet_range_wrapper(x): #x = [V,h]
    V,h = x
    # Safety checks to avoid numerical issues
    if V <= 0 or h < 0:
        return 1e10  # Return large penalty value for infeasible values
    
    rho = get_rho(h)
    
    # Calculate lift coefficient from required lift
    ...
    # Calculate drag force (this is the thrust required for steady flight)
    D = get_D(CD,rho,V,S)
    
    # Safety check for thrust
    if D <= 0:
        return 1e10
    ...
    # Safety check for specific fuel consumption
    if ct <= 0:
        return 1e10
    
    # Calculate range using Breguet equation
    range_val = brequet_range(V,ct,CL,CD,Wi,Wf_75)
    
    # Return negative range for minimization
    return -range_val

def plot_brequet_contour_with_path(func, path_history, x_range=(10, 300), y_range=(0, 25000),show_plot=False):
...
if __name__ == "__main__":
    ...
    xk,i,grad_history,path_history = quasi_newton_bfgs(x0, brequet_range_wrapper, grad_brequet, tol=1e-7, max_iter=50000, alpha=1.0, c=1e-4, rho=0.8)
    #plot log(grad_history) vs iteration
    grad_history_safe = np.maximum(grad_history, 1e-16)  # Avoid log(0)
    plt.plot(np.log(grad_history_safe))
    plt.xlabel("Iteration")
    plt.ylabel("Log(Gradient Norm)")
